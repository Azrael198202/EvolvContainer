...
public class DockerService {
    ...
    private static String run(List<String> cmd, File workDir, Duration timeout, boolean ignoreNonZero) throws Exception {
        return VueUtils.runDirect(cmd, workDir, timeout, ignoreNonZero);
    }

    // ------------------- Docker Basic -------------------
    public boolean exists(String container) throws Exception {
        List<String> cmd = List.of(dockerCmd(), "inspect", container);
        try { run(cmd, null, MID, false); return true; }
    ...
    public String build(File dir, String image) throws Exception {
        return run(List.of(dockerCmd(), "build", "-t", image, "."), dir, LONG, false);
    }
    ...
    public String runDetached(String container, int port, String image) throws Exception {
        return run(List.of(dockerCmd(), "run", "-d", "--name", container, "-p", port + ":80", image), null, MID, false);
    }
